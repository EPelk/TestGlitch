...
//End of things that will be moved later
    ...
    protected void execute() {
        ...
        if (target == null){
        ...
        else if(Math.abs(Robot.drivetrain.getContinuousHeading() - startRotation) < Math.abs(target.squint())) {
            double basePower = 0.3 * Math.signum(target.squint());
            SmartDashboard.putNumber("visionturn sign", basePower);
            Robot.drivetrain.setPower(basePower, -basePower);
            ...
            else if(target2.standoff() - ((Robot.drivetrain.getLeftDistance() + Robot.drivetrain.getRightDistance()) / 2 - startDist) > 25) {
